#Include <Misc>
; #Include "C:\Users\User\Downloads\111\ahk2_lib-master\JSON.ahk"
#Include <WebSocket>
/************************************************************************
...
class Chrome {
  ...
  static FindInstance(exename := 'Chrome.exe', debugport := 0) {
    ...
    .ExecQuery("SELECT CommandLine, ProcessID FROM Win32_Process WHERE Name = '" exename "' AND CommandLine LIKE '% --remote-debugging-port=%'")
  ...
  NewPage(url := 'about:blank', fnCallback?) {
    http := Chrome._http
    http.Open('PUT', 'http://127.0.0.1:' this.DebugPort '/json/new?' url), http.Send()
    if ((PageData := JSON.parse(http.responseText))
    .Has('webSocketDebuggerUrl'))
